...
def main() -> int:
    ...
    required_literals = (
        0x20001F80, 0x20001F84, 0x20001F88, 0x2000003C,
        0x08021DDD, 0x200000E4, 0x2000015C, 0x48000800,
        0x08010561, 0x08010B7D, 1200, 60000,
    )
    for address in required_literals:
        if blob.count(struct.pack("<I", address)) != 1:
            raise RuntimeError(f"literal 0x{address:08x} count mismatch")
    if blob.count(b"DPMRREADY\n") != 1:
        raise RuntimeError("ready marker missing or duplicated")

    disassembler = Cs(CS_ARCH_ARM, CS_MODE_THUMB | CS_MODE_LITTLE_ENDIAN)
    instructions = list(disassembler.disasm(blob, LOAD_ADDRESS))
    code_end = next(
        item.address + len(item.bytes)
        for item in instructions
        if item.mnemonic == "bx" and item.op_str == "r3")
    for item in instructions:
        if item.address >= code_end:
            break
        if len(item.bytes) == 4 and item.mnemonic != "bl":
            raise RuntimeError(
                f"non-Cortex-M0 32-bit instruction at 0x{item.address:08x}: "
    ...
    lines = [
        ...
        "startup_parser=vendor_0x0800fff8_bridge_flag_0",
        "ready_condition=0x200000e4:2",
        "ready_uart_marker=DPMRREADY\\n",
        "raw_bridge_baud=57600_unchanged",
        "startup_timeout_ticks=1200",

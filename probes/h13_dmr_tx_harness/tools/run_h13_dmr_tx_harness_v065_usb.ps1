...
$ExpectedVersionName = '0.65-first-data-before-vlc1-ack-no-rf-candidate'
$ExpectedApkSha256 = 'F85A76DA2DAD3CB506A511505928A13EB03D5B90F4F96E5B4C10345DF0DC4BCB'
$Apk = Join-Path $PSScriptRoot '..\dist\H13_DMR_TX_Harness_v0.65_FirstDataBeforeVlc1AckEvidenceNoRfCandidate.apk'
...
"@)
        ...
        try {
            ...
            [IO.File]::WriteAllBytes((Join-Path $TempTrigger `
                '0002_relay_data27_01.bin'), $Unit1)
            [IO.File]::WriteAllBytes((Join-Path $TempTrigger `
                '0003_relay_combined_trigger.bin'), [byte[]]($Unit0 + $Unit1))
            [IO.File]::WriteAllBytes((Join-Path $TempTrigger `
                '0004_relay_combined81.bin'), [byte[]]@())
            [IO.File]::WriteAllText((Join-Path $TempTrigger `
                '0005_relay_trigger_contract.bin'),
                "required_trigger_units=2`ncombined_trigger_bytes=54`ncombined81_bytes=0`n")
            $GoodResult = "relay_trigger_required_units=2`n"
            Assert-TwoUnitTriggerEvidence -Root $TempTrigger `
                -ResultText $GoodResult

            $Rejected = $false
            try {
                Assert-TwoUnitTriggerEvidence -Root $TempTrigger `
                    -ResultText "relay_trigger_required_units=3`n"
            } catch { $Rejected = $true }
            if (-not $Rejected) {
                throw '宿主两单元证据门未拒绝错误触发阈值。'
            }

            $CombinedPath = Join-Path $TempTrigger `
                '0003_relay_combined_trigger.bin'
            $Tampered = [IO.File]::ReadAllBytes($CombinedPath)
            $Tampered[53] = $Tampered[53] -bxor 0x01
            ...
            if (-not $Rejected) {
                throw '宿主两单元证据门未拒绝篡改的54字节触发原件。'
            }
            [IO.File]::WriteAllBytes($CombinedPath, [byte[]]($Unit0 + $Unit1))
            ...
            [IO.File]::WriteAllText($Meta1Path,
                "point=credit_wait`nunit_before=1`ncredit_before=0`nbegin_ms=2`nrequested_ms=1500`nreturn_ms=3`nelapsed_ms=1`nbytes=0`n")
            [IO.File]::WriteAllBytes($Raw1Path, [byte[]]@())
            $ReadResult = "data36_written=1`nrelay_credits_consumed=0`n"
            ...
            [IO.File]::WriteAllText($BoundaryPath,
                "source=vlc_1`nchunk_bytes=44`naccepted_bytes_before_write=36`ndata27_at_write=2`nvlc_acks_at_write=1`nack_ceiling=1`n")
            ...
            foreach ($Index in 2..4) {
                [IO.File]::WriteAllText((Join-Path $TempTrigger `
                    ("001$($Index)_relay_vlc_${Index}_data_gate.bin")),
                    "label=vlc_$Index`ncontrol_flush_ms=100`nack_observed_ms=120`nunits_at_control_flush=6`nunits_at_ack=6`n")
            }
            $VlcResult = "vlc_acks=5`n"
            Assert-LaterVlcDataGateEvidence -Root $TempTrigger `
                -ResultText $VlcResult
            $Gate2 = Join-Path $TempTrigger `
                '0012_relay_vlc_2_data_gate.bin'
            [IO.File]::WriteAllText($Gate2,
                ([IO.File]::ReadAllText($Gate2) -replace
                    'units_at_ack=6', 'units_at_ack=7'))
            $Rejected = $false
            try {
                Assert-LaterVlcDataGateEvidence -Root $TempTrigger `
                    -ResultText $VlcResult
            ...
            if (-not $Rejected) {
                throw '宿主VLC ACK前暂停门未拒绝提前数据写出。'
            }
        } finally {
            Remove-Item -LiteralPath $TempTrigger -Recurse -Force `
                -ErrorAction SilentlyContinue
        }
        Write-Host '宿主模式语义门、两单元门、原始读取门、首包边界门和VLC ACK暂停门故障注入自测通过。'
    } finally {
...
function Assert-PulledDeviceManifest {
    ...
    Assert-LaterVlcDataGateEvidence -Root $Root -ResultText $ResultText
...
function Assert-TwoUnitTriggerEvidence {
    param(
    ...
    if ($ThresholdMatches.Count -ne 1 -or
            $ThresholdMatches[0].Groups['value'].Value -ne '2') {
        throw '原子结果没有唯一确认两单元触发阈值。'
    }
    ...
    if ($Contracts.Count -ne 1) {
        throw "两单元触发合同原件数量错误：$($Contracts.Count)"
    }
    $ContractText = Get-Content -LiteralPath $Contracts[0].FullName -Raw
    foreach ($ExpectedLine in @('required_trigger_units=2',
            'combined_trigger_bytes=54','combined81_bytes=0')) {
        $Matches = [regex]::Matches($ContractText,
            "(?m)^$([regex]::Escape($ExpectedLine))`r?$")
        if ($Matches.Count -ne 1) {
            throw "两单元触发合同缺少唯一严格字段：$ExpectedLine"
        }
    }
    ...
    if ($Combined.Count -ne 1 -or $Combined[0].Length -ne 54) {
        throw "两单元54字节触发原件数量或长度错误：count=$($Combined.Count)"
    }
    if ($LegacyCombined.Count -ne 1 -or $LegacyCombined[0].Length -ne 0) {
        throw "两单元模式不得生成81字节触发原件：count=$($LegacyCombined.Count)"
    }

    $Units = @(Get-ChildItem -LiteralPath $Root -File |
        Where-Object { $_.Name -match '_relay_data27_\d{2}\.bin$' } |
        Sort-Object Name)
    if ($Units.Count -lt 2 -or $Units[0].Length -ne 27 -or
            $Units[1].Length -ne 27) {
        throw "两单元触发缺少前两个独立27字节原件：count=$($Units.Count)"
    }
    $ExpectedCombined = [byte[]]((
        [IO.File]::ReadAllBytes($Units[0].FullName)) + (
        [IO.File]::ReadAllBytes($Units[1].FullName)))
    Assert-ExactBytesFile -Path $Combined[0].FullName `
        -Expected $ExpectedCombined -Label '两单元54字节触发原件'
}
...
function Assert-RelayReadEvidence {
    ...
    if (-not $WrittenMatch.Success -or -not $ConsumedMatch.Success) {
        throw '原子结果缺少data36写出或信用消费计数。'
    }
    ...
    $RawFiles = @(Get-ChildItem -LiteralPath $Root -File |
        ...
        Sort-Object Name)
    if ($Written -eq 0 -and $Summaries.Count -eq 0 -and
    ...
    for ($Index = 0; $Index -lt $ExpectedSamples; $Index++) {
        ...
        if ($Meta -match '(?m)^point=credit_wait\r?$') {
            $CreditWaitSeen = $true
        }
    ...
    if ($Written -gt $Consumed -and -not $CreditWaitSeen) {
        throw '存在未消费写出，但缺少credit_wait原始读取链。'
    }
}
...
function Assert-FirstWriteBoundaryEvidence {
    ...
    foreach ($ExpectedLine in @('source=vlc_1','vlc_acks_at_write=1',
            'ack_ceiling=1')) {
        if ($Text -notmatch
                "(?m)^$([regex]::Escape($ExpectedLine))`r?$") {
    ...
    if (-not $Chunk.Success -or -not $Accepted.Success -or
            ...
            [int]$Units.Groups['value'].Value -lt 2) {
        throw '首包完整帧边界、块内偏移或实时单元计数不成立。'
    }
}

function Assert-LaterVlcDataGateEvidence {
    param(
        [Parameter(Mandatory=$true)][string]$Root,
        [Parameter(Mandatory=$true)][string]$ResultText
    )
    ...
    foreach ($Index in 2..4) {
        $Files = @(Get-ChildItem -LiteralPath $Root -File |
            Where-Object { $_.Name -match
                "_relay_vlc_${Index}_data_gate\.bin$" })
        $Required = $AckCount -ge ($Index + 1)
        if (-not $Required) {
            if ($Files.Count -ne 0) {
                throw "VLC$Index 尚未确认却出现数据恢复门凭证。"
            }
            continue
        }
        if ($Files.Count -ne 1) {
            throw "VLC$Index 数据恢复门凭证数量错误：$($Files.Count)"
        }
        $Text = Get-Content -LiteralPath $Files[0].FullName -Raw
        if ($Text -notmatch "(?m)^label=vlc_$Index`r?$") {
            throw "VLC$Index 数据恢复门标签错误。"
        }
        ...
        if (-not $Flush.Success -or -not $Ack.Success -or
                ...
                $UnitsAtAck.Groups['value'].Value) {
            throw "VLC$Index ACK前数据暂停门失败。"
        }
    }
}
...
function Assert-TripleSosEvidence {
    ...
    Assert-TwoUnitTriggerEvidence -Root $Root -ResultText $ResultText

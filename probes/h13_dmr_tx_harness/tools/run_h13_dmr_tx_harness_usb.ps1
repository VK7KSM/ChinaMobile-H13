...
$ExpectedVersionName = '0.58-triple-sos-deterministic-wire-gate-low-power-candidate'
$ExpectedApkSha256 = '881B4A4BA271C3FDF4FFE0CF89FDADCF6FD6E01AFA989AB7BB1D06100B3F261A'
$Apk = Join-Path $PSScriptRoot '..\dist\H13_DMR_TX_Harness_v0.58_TripleSosDeterministicWireGateLowPowerCandidate.apk'
...
function Assert-ModeResult {
    ...
    $ExpectedControlWrites = $Expected.Setup + $Expected.Vlc + $CleanupCount + $RetryCount
    foreach ($Pair in @(
    ...
    try {
        $Cases = @(
            ...
            @{ Mode='realtime_relay_software_privacy_triple_sos_no_rf'; DeviceMode='realtime_relay_software_privacy_triple_sos_no_rf'; Setup=5; Vlc=5; Data=78;
                First='true'; Second='false'; Prep='false'; Off='false'; Arm='false'; Deadline='false'; Sram='true' },
            @{ Mode='realtime_relay_software_privacy_triple_sos_low_power_rf'; DeviceMode='realtime_relay_software_privacy_triple_sos_low_power_rf'; Setup=5; Vlc=5; Data=78;
                First='true'; Second='false'; Prep='true'; Off='true'; Arm='true'; Deadline='true'; Sram='true' },
            @{ Mode='realtime_relay_encode_dmr_morse_unique_five_low_power_rf'; DeviceMode='realtime_relay_encode_dmr_morse_unique_five_low_power_rf'; Setup=5; Vlc=5; Data=5;
                First='true'; Second='false'; Prep='true'; Off='true'; Arm='false'; Deadline='false'; Sram='true' },
        ...
        foreach ($Case in $Cases) {
            ...
            $Cleanup = if ($Case.Mode -in @(
                    'realtime_relay_encode_dmr_morse_unique_five_low_power_rf',
                    'realtime_relay_software_privacy_triple_sos_low_power_rf')) { 2 } else { 0 }
            $Control = $Case.Setup + $Case.Vlc + $Cleanup
            ...
            $Fixture = @(
                ...
                "vlc_acks=$($Case.Vlc)", "cleanup_acks=$Cleanup", "data36_written=$($Case.Data)",
...
if ($Mode -eq 'low_power_rf') {
    throw '本版本拒绝摩尔斯低功率射频路径。'
}
if ($Mode -eq 'realtime_relay_encode_dmr_morse_unique_five_low_power_rf' -and
        $RfAuthorization -ne 'authorized_low_power_once') {
    throw '低功率空口须当次传入 -RfAuthorization authorized_low_power_once。'
}
if ($Mode -eq 'realtime_relay_software_privacy_triple_sos_low_power_rf' -and
        $RfAuthorization -ne 'authorized_low_power_once') {
    throw '三遍SOS低功率空口须当次传入 -RfAuthorization authorized_low_power_once。'
}
if ($Mode -in @('realtime_relay_one_data36_no_rf',
        ...
        'realtime_relay_software_privacy_triple_sos_no_rf',
        'realtime_relay_software_privacy_triple_sos_low_power_rf',
        'realtime_relay_encode_dmr_morse_unique_five_low_power_rf') -and -not $AllowPotentialRf) {
...
try {
    ...
    $DeviceMode = switch ($Mode) {
        'low_power_rf' { 'morse_low_power_rf' }
        ...
        'realtime_relay_encode_dmr_morse_unique_five_low_power_rf' { 'realtime_relay_encode_dmr_morse_unique_five_low_power_rf' }
    ...
    $StartArguments = @('shell','am','start','-W','-n',$Activity,
        '--es','mode',$DeviceMode)
    if ($Mode -eq 'low_power_rf') {
        $StartArguments += @('--es','rf_permission',
            'authorized_low_power_once')
    }
    if ($Mode -in @('realtime_relay_encode_dmr_morse_unique_five_low_power_rf',
            'realtime_relay_software_privacy_triple_sos_low_power_rf')) {
        $StartArguments += @('--es','rf_permission', $RfAuthorization)
    }
    ...
    if ($Result) { Assert-ModeResult $Result }
} finally {

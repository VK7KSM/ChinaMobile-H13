...
$ExpectedVersionCode = 75
$ExpectedVersionName = '0.75-post-vlc-three-live-units-one-data36-no-rf'
$ExpectedApkSha256 = 'F246D0F3B98C56CDAD935AAC1C704274F31A63A46CB111156BD076BBE5F17464'
$Apk = Join-Path $PSScriptRoot '..\dist\H13_DMR_TX_Harness_v0.75_PostVlcThreeLiveUnitsOneData36NoRf.apk'
...
function Assert-ModeResult {
    ...
    $Expected = switch ($Mode) {
        ...
        'realtime_post_vlc_three_live_units_software_one_data36_no_rf' { @{
            Mode='realtime_post_vlc_three_live_units_software_one_data36_no_rf'; Setup=5; Vlc=5; Data=1 } }
        'realtime_relay_software_privacy_triple_sos_low_power_rf' { @{
    ...
    if ($Mode -eq 'realtime_post_vlc_three_live_units_software_one_data36_no_rf') {
        if ($ResultText -notmatch '(?m)^relay_payload_source=post_vlc_software_privacy_morse_first_data36\r?$' -or
                $ResultText -notmatch '(?m)^relay_units_written=1\r?$' -or
                $ResultText -notmatch '(?m)^relay_post_arm_data27_total=(?:[3-9]|[1-9][0-9]+)\r?$' -or
                $ResultText -notmatch '(?m)^post_vlc_observation_class=(?:ONE_DATA36_SHORT_CREDIT|ONE_DATA36_CHAN_D_CONTINUATION|ONE_DATA36_MIXED_RESPONSE|ONE_DATA36_SILENT)\r?$') {
            throw '模式语义门失败：VLC后第三新鲜单元单包证据或观察分类不完整'
        }
    }
    ...
    try {
        $Cases = @(
            ...
            @{ Mode='realtime_relay_software_privacy_triple_sos_no_rf'; DeviceMode='realtime_relay_software_privacy_triple_sos_no_rf'; Setup=5; Vlc=5; Data=78;
                First='true'; Second='false'; Prep='false'; Off='false'; Arm='false'; Deadline='false'; Sram='true' },
            @{ Mode='realtime_post_vlc_three_live_units_software_one_data36_no_rf'; DeviceMode='realtime_post_vlc_three_live_units_software_one_data36_no_rf'; Setup=5; Vlc=5; Data=1; Credits=0;
                First='true'; Second='false'; Prep='false'; Off='false'; Arm='false'; Deadline='false'; Sram='true' },
        ...
        foreach ($Case in $Cases) {
            ...
            } elseif ($Case.Mode -eq 'realtime_post_vlc_three_live_units_software_one_data36_no_rf') {
            ...
            $Fixture = @(
                ...
                "relay_post_arm_data27_total=$(if ($Case.Mode -eq 'realtime_post_vlc_three_live_units_software_one_data36_no_rf') { '3' } else { '0' })",
                "post_vlc_observation_class=$(if ($Case.Mode -eq 'realtime_post_vlc_three_live_units_software_one_data36_no_rf') { 'ONE_DATA36_SILENT' } else { 'NOT_APPLICABLE' })",
...
"@)
        ...
        Write-Host '宿主模式语义门、一单元门、原始读取门、首包边界门、连续正文完成门和VLC期间数据保持门故障注入自测通过。'
    } finally {
...
if ($Mode -in @('realtime_relay_one_data36_no_rf',
        ...
        'realtime_relay_software_privacy_triple_sos_no_rf',
        'realtime_post_vlc_three_live_units_software_one_data36_no_rf',
        'realtime_relay_software_privacy_triple_sos_low_power_rf') -and -not $AllowPotentialRf) {
...
try {
    ...
    $DeviceMode = switch ($Mode) {
        ...
        'realtime_post_vlc_three_live_units_software_one_data36_no_rf' { 'realtime_post_vlc_three_live_units_software_one_data36_no_rf' }
    ...
    $StartArguments = @('shell','am','start','-W','-n',$Activity,
        '--es','mode',$DeviceMode)
    if ($Mode -eq 'realtime_relay_software_privacy_triple_sos_low_power_rf') {

...
$ExpectedVersionName = '0.80-ack-paced-triple-sos-low-power-rf-candidate'
$ExpectedApkSha256 = '613C9A8A0AAE0B570C61D7F0CD8FB21C4F43BF68CFFA53EFC7BEC5B93727590A'
$Apk = Join-Path $PSScriptRoot '..\dist\H13_DMR_TX_Harness_v0.80_AckPacedTripleSosLowPowerRfCandidate.apk'
...
function Assert-ModeResult {
    ...
    if ($Mode -eq 'realtime_relay_software_privacy_triple_sos_no_rf' -and
    ...
    if ($Mode -in @('ack_paced_vlc_software_triple_sos_active_no_rf',
            'realtime_relay_software_privacy_triple_sos_low_power_rf')) {
    ...
    try {
        $Cases = @(
            ...
            @{ Mode='ack_paced_vlc_software_triple_sos_active_no_rf'; DeviceMode='ack_paced_vlc_software_triple_sos_active_no_rf'; Setup=5; Vlc=5; Data=78; Credits=0;
                First='true'; Second='false'; Prep='false'; Off='false'; Arm='false'; Deadline='false'; Sram='true' },
            @{ Mode='realtime_relay_software_privacy_triple_sos_low_power_rf'; DeviceMode='realtime_relay_software_privacy_triple_sos_low_power_rf'; Setup=5; Vlc=5; Data=78; Credits=0;
        ...
        foreach ($Case in $Cases) {
            ...
            $Fixture = @(
                ...
                "relay_trigger_required_units=$(if ($Case.Mode -in @('ack_paced_vlc_software_one_data36_no_rf','ack_paced_vlc_software_triple_sos_active_no_rf','realtime_relay_software_privacy_triple_sos_low_power_rf')) { '0' } else { '3' })",
...
"@)
        ...
        try {
            ...
            $TempActiveTiming = Join-Path ([IO.Path]::GetTempPath()) `
                ('h13-v080-active-timing-' + [guid]::NewGuid().ToString('N'))
...
function Assert-RelayReadEvidence {
    ...
    if ($Mode -ne 'realtime_relay_software_privacy_triple_sos_no_rf') {
        return
    }

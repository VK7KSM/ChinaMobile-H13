...
$ExpectedVersionName = '0.68-vlc-ack-and-live-unit-resume-no-rf-candidate'
$ExpectedApkSha256 = '1E169512358FE75808CFC03F30CA2B1EA184EB39472B3148D6D993AF7B6F3CCC'
$Apk = Join-Path $PSScriptRoot '..\dist\H13_DMR_TX_Harness_v0.68_VlcAckAndLiveUnitResumeNoRfCandidate.apk'
        ...
        try {
            ...
            foreach ($Index in 1..4) {
                [IO.File]::WriteAllText((Join-Path $TempTrigger `
                    ...
                    "label=vlc_$Index`ncontrol_flush_ms=100`nack_observed_ms=120`nfirst_post_control_data27_ms=130`nresume_ready_ms=130`nunits_at_control_flush=6`nunits_at_ack=6`nunits_at_resume=6`ncredits_at_resume=6`ndata27_at_control_flush=10`nfirst_post_control_data27_count=11`n")
            }
            ...
            $Gate1 = Join-Path $TempTrigger `
                '0011_relay_vlc_1_data_gate.bin'
            ...
            if (-not $Rejected) {
                throw '宿主VLC恢复门未拒绝ACK前提前数据写出。'
            }
            [IO.File]::WriteAllText($Gate1,
                ([IO.File]::ReadAllText($Gate1) -replace
                    'units_at_ack=7', 'units_at_ack=6'))
            [IO.File]::WriteAllText($Gate1,
                ([IO.File]::ReadAllText($Gate1) -replace
                    'first_post_control_data27_count=11',
                    'first_post_control_data27_count=10'))
            $Rejected = $false
            try {
                Assert-LaterVlcDataGateEvidence -Root $TempTrigger `
                    -ResultText $VlcResult
            ...
            if (-not $Rejected) {
                throw '宿主VLC恢复门未拒绝缺少新实时单元的伪证据。'
            }
        } finally {
            Remove-Item -LiteralPath $TempTrigger -Recurse -Force `
                -ErrorAction SilentlyContinue
        }
        Write-Host '宿主模式语义门、一单元门、原始读取门、首包边界门和VLC确认加新实时单元恢复门故障注入自测通过。'
    } finally {
...
function Assert-PulledDeviceManifest {
    ...
    Save-Text 'device_vlc_resume_gate_verification.txt' `
        '通过：现有VLC恢复门凭证均满足严格确认与控制后新实时单元双条件；PASS会话要求VLC1至VLC4四份凭证齐全。'
...
function Assert-LaterVlcDataGateEvidence {
    ...
    foreach ($Index in 1..4) {
        ...
        $Data27 = [regex]::Match($Text,
            '(?m)^first_post_control_data27_ms=(?<value>\d+)\r?$')
        $Resume = [regex]::Match($Text,
            '(?m)^resume_ready_ms=(?<value>\d+)\r?$')
        $UnitsAtFlush = [regex]::Match($Text,
            '(?m)^units_at_control_flush=(?<value>\d+)\r?$')
        $UnitsAtAck = [regex]::Match($Text,
            '(?m)^units_at_ack=(?<value>\d+)\r?$')
        $UnitsAtResume = [regex]::Match($Text,
            '(?m)^units_at_resume=(?<value>\d+)\r?$')
        $CreditsAtResume = [regex]::Match($Text,
            '(?m)^credits_at_resume=(?<value>\d+)\r?$')
        $Data27AtFlush = [regex]::Match($Text,
            '(?m)^data27_at_control_flush=(?<value>\d+)\r?$')
        $FirstData27Count = [regex]::Match($Text,
            '(?m)^first_post_control_data27_count=(?<value>\d+)\r?$')
        if (-not $Flush.Success -or -not $Ack.Success -or
                -not $Data27.Success -or -not $Resume.Success -or
                -not $UnitsAtFlush.Success -or -not $UnitsAtAck.Success -or
                -not $UnitsAtResume.Success -or
                -not $CreditsAtResume.Success -or
                -not $Data27AtFlush.Success -or
                -not $FirstData27Count.Success -or
                [long]$Ack.Groups['value'].Value -lt
                [long]$Flush.Groups['value'].Value -or
                [long]$Data27.Groups['value'].Value -lt
                [long]$Flush.Groups['value'].Value -or
                [long]$Resume.Groups['value'].Value -lt
                [long]$Ack.Groups['value'].Value -or
                [long]$Resume.Groups['value'].Value -lt
                [long]$Data27.Groups['value'].Value -or
                $UnitsAtFlush.Groups['value'].Value -ne
                $UnitsAtAck.Groups['value'].Value -or
                $UnitsAtFlush.Groups['value'].Value -ne
                $UnitsAtResume.Groups['value'].Value -or
                [int]$CreditsAtResume.Groups['value'].Value -gt
                [int]$UnitsAtResume.Groups['value'].Value -or
                [int]$FirstData27Count.Groups['value'].Value -le
                [int]$Data27AtFlush.Groups['value'].Value) {
            throw "VLC$Index 确认加新实时单元恢复门失败。"
        }
    }
}

...
$ExpectedVersionName = '0.69-first-credit-then-all-vlc-no-rf-candidate'
$ExpectedApkSha256 = '9395836B158FA430232AA819DDA93A8E3F942E07E090E3B75FCCEFFB541AC59A'
$Apk = Join-Path $PSScriptRoot '..\dist\H13_DMR_TX_Harness_v0.69_FirstCreditThenAllVlcNoRfCandidate.apk'
...
"@)
        ...
        try {
            ...
            foreach ($Index in 1..4) {
                [IO.File]::WriteAllText((Join-Path $TempTrigger `
                    ...
                    "label=vlc_$Index`ncontrol_flush_ms=100`nack_observed_ms=120`nunits_at_control_flush=1`nunits_at_ack=1`nunits_at_window_end=1`ncredits_at_window_end=1`ndata27_at_window_end=10`n")
            }
            [IO.File]::WriteAllText((Join-Path $TempTrigger `
                '0015_relay_post_vlc_resume.bin'),
                "resume_ms=140`nvlc_acks=5`nunits_at_resume=1`ncredits_at_resume=1`n")
            ...
            [IO.File]::WriteAllText($Gate1,
                ([IO.File]::ReadAllText($Gate1) -replace
                    'units_at_ack=7', 'units_at_ack=1'))
            [IO.File]::WriteAllText($Gate1,
                ([IO.File]::ReadAllText($Gate1) -replace
                    'units_at_window_end=1',
                    'units_at_window_end=2'))
            $Rejected = $false
            try {
                Assert-LaterVlcDataPauseEvidence -Root $TempTrigger `
                    -ResultText $VlcResult
        ...
        Write-Host '宿主模式语义门、一单元门、原始读取门、首包边界门和VLC期间数据暂停门故障注入自测通过。'
    } finally {
...
function Assert-PulledDeviceManifest {
    ...
    Save-Text 'device_vlc_data_pause_verification.txt' `
        '通过：现有VLC1至VLC4凭证均证明确认窗口内data36写计数保持为首包1；PASS会话要求四份凭证及五VLC后恢复凭证齐全。'
...
function Assert-LaterVlcDataPauseEvidence {
    ...
    foreach ($Index in 1..4) {
        ...
        if (-not $Flush.Success -or -not $Ack.Success -or
                ...
                [int]$UnitsAtFlush.Groups['value'].Value -ne 1 -or
                $UnitsAtFlush.Groups['value'].Value -ne
                $UnitsAtAck.Groups['value'].Value -or
                $UnitsAtFlush.Groups['value'].Value -ne
                $UnitsAtEnd.Groups['value'].Value -or
                [int]$CreditsAtEnd.Groups['value'].Value -ne 1 -or
                [int]$Data27AtEnd.Groups['value'].Value -lt 1) {
            throw "VLC$Index 确认窗口未保持只有首包，数据暂停门失败。"
        }
    }
    if ($PassResult) {
        $ResumeFiles = @(Get-ChildItem -LiteralPath $Root -File |
            Where-Object { $_.Name -match
                '_relay_post_vlc_resume\.bin$' })
        if ($ResumeFiles.Count -ne 1) {
            throw "五VLC后恢复凭证数量错误：$($ResumeFiles.Count)"
        }
        $ResumeText = Get-Content -LiteralPath $ResumeFiles[0].FullName -Raw
        foreach ($ExpectedLine in @('vlc_acks=5','units_at_resume=1',
                'credits_at_resume=1')) {
            if ($ResumeText -notmatch
                    "(?m)^$([regex]::Escape($ExpectedLine))`r?$") {
                throw "五VLC后恢复凭证缺少严格字段：$ExpectedLine"
            }
        }
        if ($ResumeText -notmatch '(?m)^resume_ms=\d+\r?$') {
            throw '五VLC后恢复凭证缺少有效时刻。'
        }
    }
}

...
$ExpectedVersionCode = 77
$ExpectedVersionName = '0.77-ack-paced-vlc-one-data36-no-rf'
$ExpectedApkSha256 = '705C44266D4C1043FE87EAB90C4072FD86A4663F185CEB47821F80BBB7AC4A76'
$Apk = Join-Path $PSScriptRoot '..\dist\H13_DMR_TX_Harness_v0.77_AckPacedVlcOneData36NoRf.apk'
    ...
    try {
        ...
        foreach ($Case in $Cases) {
            ...
            $Fixture = @(
                ...
                "relay_trigger_required_units=$(if ($Case.Mode -eq 'ack_paced_vlc_software_one_data36_no_rf') { '0' } else { '3' })",
...
"@)
        ...
        Write-Host '宿主模式语义门、一单元门、原始读取门、首包边界门、连续正文完成门、VLC期间数据保持门和ACK节拍门故障注入自测通过。'
    } finally {
...
function Assert-AckPacedVlcEvidence {
    ...
    if ($Mode -ne 'ack_paced_vlc_software_one_data36_no_rf') {
        return
    }
    foreach ($ExpectedLine in @('data36_written=1',
            'relay_units_written=1','relay_credits_consumed=1',
            'relay_trigger_required_units=0')) {

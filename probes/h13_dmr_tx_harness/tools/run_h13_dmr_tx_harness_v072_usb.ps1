...
$ExpectedVersionCode = 72
$ExpectedVersionName = '0.72-continuous-body-sixteen-second-low-power-rf-candidate'
$ExpectedApkSha256 = 'C25E18677BE87687ED2AC3773314208F007138BA7FB7867D0F34215A6EC1E899'
$Apk = Join-Path $PSScriptRoot '..\dist\H13_DMR_TX_Harness_v0.72_ContinuousBodySixteenSecondLowPowerRfCandidate.apk'
...
"@)
        ...
        $TempTimingNames = Join-Path ([IO.Path]::GetTempPath()) `
            ('h13-v072-timing-names-' + [guid]::NewGuid().ToString('N'))
        ...
        if ($Header -notmatch
                'realtime_relay_software_privacy_triple_sos_low_power_rf') {
                throw 'v0.72公开参数白名单缺少唯一批准的三遍SOS低功率模式。'
        }
        if ([regex]::Matches($Header, "'[^']*low_power_rf'").Count -ne 1) {
                throw 'v0.72公开参数白名单只能包含一个低功率射频模式。'
        }
